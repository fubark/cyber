...
args = os.parseArgs({'o', 'libPath', 'stripPrefix'})
prefix = args.get('stripPrefix') ?else 'DONT_MATCH'
reserved_keywords = {
    type = true,
    'Fn' = true,
}
...
libPath := if (found_existing_lib_path) 'libPath' else "'%{args.get('libpath') ?else 'lib.dll'}'"
