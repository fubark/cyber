    ...
    switch (left.type()) {
        ...
        .accessExpr => {
            ...
            if (leftRes.resType == .local) {
                ...
                const rightExpr = Expr{ .nodeId = rightId, .hasTypeCstr = true, .reqTypeCstr = !leftT.dynamic, .preferType = leftT.id };
...
pub fn declareHostStruct(c: *cy.Chunk, nodeId: cy.NodeId) !*cy.sym.Sym {
    const node = c.ast.node(nodeId);
    ...
    log.tracev("Invoke type loader for: {s}", .{name});
    if (!typeLoader(@ptrCast(c.compiler.vm), info, &res)) {
        return c.reportErrorAt("Failed to load #host type `{}` struct.", &.{v(name)}, nodeId);
    }
    ...
    switch (res.type) {
        ...
        cc.BindTypePredefined => {
            const type_e = c.sema.types.items[res.data.predefined.typeId];
            if (type_e.sym.type != .core_t and type_e.sym.type != .struct_t) {
                return c.reportErrorAt("Expected predefined struct type.", &.{}, nodeId);
            }
            return type_e.sym;
        },
        else => {
...
pub fn declareStruct(c: *cy.Chunk, nodeId: cy.NodeId) !*cy.Sym {
    ...
    if (header.head.data.objectHeader.modHead != cy.NullNode) {
        const modifier = c.ast.node(header.head.data.objectHeader.modHead);
        if (modifier.data.dirModifier.type == .host) {
            return @ptrCast(try declareHostStruct(c, nodeId));
        }
    }
...
fn callSym(c: *cy.Chunk, preIdx: u32, sym: *Sym, numArgs: u8, symNodeId: cy.NodeId, argHead: cy.NodeId) !ExprResult {
    ...
    switch (sym.type) {
        ...
        .core_t => {
            const type_id = sym.cast(.core_t).type;
            return callTypeSym(c, preIdx, sym, numArgs, symNodeId, argHead, type_id);
        }, 
        .custom_object_t => {
    ...
    if (symAsValue) {
        ...
        switch (sym.type) {
            ...
            .core_t => {
                ...
                const irIdx = try c.ir.pushExpr(.typeSym, c.alloc, typeId, nodeId, .{ .typeId = core_t });
                return ExprResult.init(irIdx, ctype);
            },
            .enumMember => {
...
fn visitChunkInit(self: *cy.Compiler, c: *cy.Chunk) !void {
    ...
    const exprLoc = try mainChunk.ir.pushExpr(.preCallFuncSym, c.alloc, bt.None, cy.NullNode, .{ .callFuncSym = .{
...
fn semaSwitchExpr(c: *cy.Chunk, nodeId: cy.NodeId) !ExprResult {
    ...
    var exprLoc: u32 = undefined;
    if (info.exprIsChoiceType) {
        blockExprLoc = try c.ir.pushExpr(.blockExpr, c.alloc, undefined, nodeId, .{ .bodyHead = cy.NullId });
        try pushBlock(c, nodeId);
...
pub const ChunkExt = struct {
    ...
    pub fn semaExpr(c: *cy.Chunk, nodeId: cy.NodeId, opts: SemaExprOptions) !ExprResult {
        ...
        const expr = Expr{
            .preferType = opts.target_t,
            .hasTypeCstr = opts.has_target_t,
    ...
    pub fn semaExprNoCheck(c: *cy.Chunk, expr: Expr) anyerror!ExprResult {
        ...
        switch (node.type()) {
            ...
            .symbolLit => {
                const name = c.ast.nodeString(node);
                if (expr.hasTypeCstr and c.sema.isEnumType(expr.preferType)) {
                    const sym = c.sema.getTypeSym(expr.preferType).cast(.enum_t);
                    if (sym.getMemberTag(name)) |tag| {
                        const irIdx = try c.ir.pushExpr(.enumMemberSym, c.alloc, expr.preferType, nodeId, .{
    ...
    pub fn semaCallFuncSym(c: *cy.Chunk, loc: u32, calleeId: cy.NodeId, funcSym: *cy.sym.FuncSym, args: StackTypesResult) !ExprResult {
        const reqRet = bt.Any;
...
fn assignToLocalVar(c: *cy.Chunk, localRes: ExprResult, rhs: cy.NodeId, opts: AssignOptions) !ExprResult {
    ...
    const rightExpr = Expr{ .preferType = svar.vtype.id, .nodeId = rhs, .hasTypeCstr = true, .reqTypeCstr = !svar.vtype.dynamic };
    const right = try c.semaExprOrOpAssignBinExpr(rightExpr, opts.rhsOpAssignBinExpr);
